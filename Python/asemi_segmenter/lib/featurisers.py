...
#########################################
class HistogramFeaturiser2(Featuriser):
    '''
...
class HistogramFeaturiser(Featuriser):
    ...
    def get_num_steps_to_featurise(self, data_scales, block_rows, block_cols, row_range=slice(None), col_range=slice(None)):
        ...
        :rtype: int
        '''
        return arrayprocs.get_num_blocks_in_data(data_scales[0].shape, (block_rows, block_cols), self.context_needed)
    ...
    def featurise(self, data_scales, slice_index, block_rows, block_cols, row_range=slice(None), col_range=slice(None), output=None, output_start_row_index=0, output_start_col_index=0, n_jobs=1):
        ...
        def processor(params, use_voxel_value, histogram_params, feature_size, full_input_ranges, output_start_row_index, output_start_col_index):
            '''Processor for process_array_in_blocks_single_slice.'''
            [ num_rows_out, num_cols_out ] = params[0]['contextless_shape']
            ...
            out_indexes = (
                    ...
                    slice(output_start_col_index, output_start_col_index+feature_size)
                )
        ...
        return arrayprocs.process_array_in_blocks_single_slice(
            ...
            n_jobs=n_jobs,
            extra_params=(self.use_voxel_value, self.histogram_params, self.feature_size, (row_range, col_range), output_start_row_index, output_start_col_index),
            )